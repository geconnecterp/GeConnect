...
{
	public class R035_SaldoDeCuentas : Servicio<EntidadBase>, IGeneradorReporte
	{
		...
		public R035_SaldoDeCuentas(IUnitOfWork uow, IConsultaServicio consulta, IFinancieroServicio financieroServicio,
		...
		public string Generar(ReporteSolicitudDto solicitud)
		{
			...
			try
			{
				...
				string tit;
				List<SaldoDeCuentaDto> registros = ObtenerDatos(solicitud, out tit);
				...
				#region Lista de Saldos en Cuenta
				HelperPdf.CargarTablaSaldosEnCuenta(pdf, registros, chico, normalBold);
				#endregion
		...
		private List<SaldoDeCuentaDto> ObtenerDatos(ReporteSolicitudDto solicitud, out string titulo)
		{
			var fHastaPrint = solicitud.Parametros.GetValueOrDefault("hasta1Print", "").ToString() ?? DateTime.Now.ToString("dd-MM-yyyy");
			var hasta = solicitud.Parametros.GetValueOrDefault("hasta", "").ToString() ?? DateTime.Now.ToString("dd-MM-yyyy");

			titulo = $"Informe de Saldos al {fHastaPrint} Valores en Cartera y al Cobro";

			return _financieroServicio.GetSaldoDeCuentas(new BuscarSaldoDeCuentasRequest() 
			{ 
