...
{
	public class R091_Comisiones_Repartidores_Resumen : Servicio<EntidadBase>, IGeneradorReporte
	{
		...
		public R091_Comisiones_Repartidores_Resumen(IUnitOfWork uow, IConsultaServicio consSrv,
		...
		public string Generar(ReporteSolicitudDto solicitud)
		{
			...
			try
			{
				...
				string tit;
				string subtit;
				List<ComisionesDeRepartidoresResumenDto> registros = ObtenerDatos(solicitud, out tit, out subtit);
				...
				CargarComisionDeRepartidoresResumen(pdf, registros, chico, normal, normalBold, titulo, tituloBig);
				#endregion
		...
		public static void CargarComisionDeRepartidoresResumen(Document pdf, List<ComisionesDeRepartidoresResumenDto> registros, Font chico, Font normal, Font normalBold, Font titulo, Font tituloBig)
		{
			...
			// ============================================================
			AddHeader(tbl, "Repartidor", normalBold, Element.ALIGN_LEFT);
			...
			foreach (var r in registros.OrderBy(x => x.rp_id))
			{
				string repartidor = $"({r.rp_id}) {r.rp_nombre}";
				...
				AddCell(tbl, repartidor, normal, Element.ALIGN_LEFT);
		...
		private List<ComisionesDeRepartidoresResumenDto> ObtenerDatos(ReporteSolicitudDto solicitud, out string titulo, out string subtitulo)
		{
			try
			{
				var ret = new List<ComisionesDeRepartidoresResumenDto>();
				...
				ret = _consSrv.BuscarComisionDeRepartidorResumen(new ComisionesDeRepartidoresRequest()
				{
					Hasta = hasta,
					Desde = desde
				});
				titulo = $"Resumen de Comisiones por Repartidores";
