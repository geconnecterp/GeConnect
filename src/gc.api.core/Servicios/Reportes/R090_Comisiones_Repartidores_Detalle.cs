...
{
	public class R090_Comisiones_Repartidores_Detalle : Servicio<EntidadBase>, IGeneradorReporte
	{
		...
		public R090_Comisiones_Repartidores_Detalle(IUnitOfWork uow, IConsultaServicio consSrv,
		...
		public string Generar(ReporteSolicitudDto solicitud)
		{
			...
			try
			{
				...
				string tit;
				string subtit;
				List<ComisionesDeRepartidoresDetalleDto> registros = ObtenerDatos(solicitud, out tit, out subtit);
				...
				CargarComisionDeRepartidoresDetalle(pdf, registros, chico, normal, normalBold, titulo, tituloBig);
				#endregion
			...
			{
				_logger.LogError(ex, "Error en R090");
				throw new NegocioException("Se produjo un error al intentar generar el Reporte. Para mayores datos ver el log.");
			}
		}

		#region Funciones de generacion de secciones de reportes
		public static void CargarComisionDeRepartidoresDetalle(Document pdf, List<ComisionesDeRepartidoresDetalleDto> registros, Font chico, Font normal, Font normalBold, Font titulo, Font tituloBig)
		{
			...
			var gruposVendedor = registros
				.GroupBy(x => new { x.rp_id, x.rp_nombre })
				.OrderBy(g => g.Key.rp_id);
			...
			foreach (var grupoV in gruposVendedor)
			{
				...
				phrVend.Add(new Chunk("Repartidor: ", normalBold));
				phrVend.Add(new Chunk($"({grupoV.Key.rp_id}) {grupoV.Key.rp_nombre}", normalBold));
				...
				foreach (var r in lista.OrderBy(x => x.pc_fecha))
				{
					...
					AddCell(tbl, r.rp_comi_base.ToString("N2"), normal, Element.ALIGN_RIGHT);

					AddCell(tbl, r.rp_comi_porc.ToString("N2"), normal, Element.ALIGN_RIGHT);
		...
		private List<ComisionesDeRepartidoresDetalleDto> ObtenerDatos(ReporteSolicitudDto solicitud, out string titulo, out string subtitulo)
		{
			try
			{
				var ret = new List<ComisionesDeRepartidoresDetalleDto>();
				...
				ret = _consSrv.BuscarComisionDeRepartidorDetalle(new ComisionesDeRepartidoresRequest()
				{
					Hasta = hasta,
					Desde = desde
				});
				titulo = $"Detalle de Comisiones por Repartidores";
				subtitulo = $"Desde: {desde:dd/MM/yyyy} Hasta: {hasta:dd/MM/yyyy}";
				return ret;
			}
			catch (Exception)
			{
