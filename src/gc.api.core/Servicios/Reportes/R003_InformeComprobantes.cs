...
{
    public class R003_InformeComprobantes : Servicio<EntidadBase>, IGeneradorReporte
    {
        ...
        public R003_InformeComprobantes(IUnitOfWork uow, IConsultaServicio consulta,
           IOptions<EmpresaGeco> empresa, ICuentaServicio consultaSv, ILogger logger) : base(uow)
        {
            _consultaServicio = consulta;

            _empresaGeco = empresa.Value;
            _titulos = new List<string> { "Mes", "Tot.Neto", "Tot.Fact", "Dif.Mes Ant" };
            _campos = new List<string> { "Mes", "TotNeto", "TotFact", "DifMesAnt" };
        ...
        public string Generar(ReporteSolicitudDto solicitud)
        {
            ...
            try
            {
                ...
                string ctaId;
                string tit;
                List<ConsCompTotDto> registros = ObtenerDatos(solicitud, out ctaId,out tit);

                if (registros == null || registros.Count == 0)
                {
                    throw new NegocioException($"No se encontraron registros de la cuenta corriente {ctaId}.");
                }
                var neto = registros.Sum(x => x.Cm_total);
                var tot = registros.Sum(x => x.Nc_cm_total);
        ...
        private List<ConsCompTotDto> ObtenerDatos(ReporteSolicitudDto solicitud, out string ctaId,out string titulo)
        {
            //Se obtienen los parámetros del reporte
            ctaId = solicitud.Parametros.GetValueOrDefault("ctaId", "").ToString() ?? "";
            int meses = solicitud.Parametros.GetValueOrDefault("meses", "").ToInt();
            ...
            titulo = $"Compras de los últimos {meses} meses\r\nCuenta: ({ctaId}) ";
            return _consultaServicio.ConsultaComprobantesMeses(ctaId, meses, relCuil, userId);
