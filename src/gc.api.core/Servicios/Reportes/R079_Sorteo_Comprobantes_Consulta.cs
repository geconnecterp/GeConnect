...
{
	public class R079_Sorteo_Comprobantes_Consulta : Servicio<EntidadBase>, IGeneradorReporte
	{
		...
		public R079_Sorteo_Comprobantes_Consulta(IUnitOfWork uow, IApiVentasServicio ventasSv,
		...
		public string Generar(ReporteSolicitudDto solicitud)
		{
			...
			try
			{
				...
				string tit;
				List<SorteoComptesDto> registros = ObtenerDatos(solicitud, out tit);
				...
				#region Lista 
				HelperPdf.CargarRepoSorteoCompteLista(pdf, registros, chico, normal, normalBold, titulo, tituloBig);
				#endregion
		...
		private List<SorteoComptesDto> ObtenerDatos(ReporteSolicitudDto solicitud, out string titulo)
		{
			try
			{
				var so_sorteo = solicitud.Parametros.GetValueOrDefault("so_sorteo", "")?.ToString() ?? null;
				var request = new SorteoCompteRequest()
				{
					so_sorteo = so_sorteo
				};
				var listaTemp = _ventasSv.ObtenerSorteoComptesLista(request);
				titulo = $"Reporte Sorteo Comprobantes N°: {so_sorteo}";
				return listaTemp;
			}
			catch (Exception)
			{
