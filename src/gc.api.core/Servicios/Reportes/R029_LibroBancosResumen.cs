...
{
	public class R029_LibroBancosResumen : Servicio<EntidadBase>, IGeneradorReporte
	{
		...
		public R029_LibroBancosResumen(IUnitOfWork uow, IConsultaServicio consulta, IFinancieroServicio financieroServicio,
		...
		public string Generar(ReporteSolicitudDto solicitud)
		{
			...
			try
			{
				...
				string tit;
				string fHasta;
				List<FinancieroBcoLibroResumenDto> registros = ObtenerDatos(solicitud, out tit, out fHasta);

				//COMPLETAMOS EL TITULO DEL REPORTE AGREGANDO LA DENOMINACIÓN DE LA CUENTA
				solicitud.Titulo = tit;
				solicitud.SubTitulo = $"Fecha hasta {fHasta}";
				...
				#region Libro Banco Resumen
				HelperPdf.CargarTablaLibroBancoResumenFinancieros(pdf, registros, chico, normalBold);
				#endregion
		...
		private List<FinancieroBcoLibroResumenDto> ObtenerDatos(ReporteSolicitudDto solicitud, out string titulo, out string fHastaPrint)
		{
			...
			titulo = $"Consulta Libro Banco Resumen";
			return _financieroServicio.GetFinancieroBcoLibroResumen(new FinancieroBcoLibroResumenRequest() 
			{ 
				ctaf_id = ctaf_id,
