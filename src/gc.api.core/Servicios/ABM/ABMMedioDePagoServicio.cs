...
{
	public class ABMMedioDePagoServicio : Servicio<Instrumento>, IABMMedioDePagoServicio
	{
		public ABMMedioDePagoServicio(IUnitOfWork uow, IOptions<PaginationOptions> pag) : base(uow, pag)
		{

		}

		public List<ABMMedioDePagoSearchDto> Buscar(QueryFilters filtros)
		{
			...
			string sp = ConstantesGC.StoredProcedures.SP_ABM_MEDIOS_PAGOS_LISTA;
			...
			if (!string.IsNullOrEmpty(filtros.Id))
			{
				...
				ps.Add(new SqlParameter("@id_desde", filtros.Id));

				if (!string.IsNullOrEmpty(filtros.Id2))
				{
					ps.Add(new SqlParameter("@id_hasta", filtros.Id2));
				}
				else
				{
					ps.Add(new SqlParameter("@id_hasta", filtros.Id));
				}
			}
			else
			{
			...
			if (filtros.Rel01 != null && filtros.Rel01.Count > 0)
			{
				...
				ps.Add(new SqlParameter("@tcf_id", sb.ToString()));
			}
			//else
			//{
