...
namespace gc.api.core.Servicios
{
    public class ApiProductoServicio : Servicio<Producto>, IApiProductoServicio
    {
        ...
        public List<ProductoDetalleDto> Obtener_ProductoDetalleBase(QueryFilters filtro)
        {
            var sp = ConstantesGC.StoredProcedures.SP_PROD_DETALLE_PROD;
            var ps = new List<SqlParameter>();

            //si o si tiene que venir la cuenta del proveedor
            var ctaId = filtro.Rel01?.First();
            ps.Add(new SqlParameter("@cta_id", ctaId));

            //identifica si tiene identificadores de productos
            if (string.IsNullOrEmpty(filtro.Id) && string.IsNullOrEmpty(filtro.Id2))
            {
                ps.Add(new SqlParameter("@id", false));
            }
            else
            {
                ...
                {
                    ps.Add(new SqlParameter("@id", true));
                    ps.Add(new SqlParameter("@p_id_d", filtro.Id));
                    ps.Add(new SqlParameter("@p_id_h", filtro.Id));
                }
                else if (!string.IsNullOrEmpty(filtro.Id) && !string.IsNullOrEmpty(filtro.Id2))
                {
                    ps.Add(new SqlParameter("@id", true));
                    ps.Add(new SqlParameter("@p_id_d", filtro.Id));
