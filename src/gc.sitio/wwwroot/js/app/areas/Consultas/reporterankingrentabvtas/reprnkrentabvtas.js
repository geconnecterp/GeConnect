$(function () {
	...
	$("#btnBuscar").on("click", function () {
		if (ValidarRangoFechas()) {
			BuscarRankingRentabDeVentas();
		}
	});

});

function BuscarRankingRentabDeVentas() {
	AbrirWaiting();
	var lSuc = [];
	var lProv = [];
	var lFam = [];
...
function ValidarRangoFechas() {
	...
	const dMax = new Date(dDesde);
	dMax.setMonth(dMax.getMonth() + 3); // suma 3 meses exactos

	if (dHasta > dMax) {
		//AbrirMensaje("ATENCIÓN", "El rango de fechas no puede superar los 60 días.");
		AbrirMensaje("ATENCIÓN", "El rango de fechas no puede superar los 3 meses.", function () {
			$("#msjModal").modal("hide");
			return false;
		}, false, ["Aceptar"], "error!", null);
		return false;
...
function ImprimirListaProductosStk_Generada() {
	...
	setTimeout(() => {
		...
		cargarReporteEnArre(92, data, "REPORTE DE RANKING Y RENTABILIDAD", "", "");
		invocacionGestorDoc({});
	}, 500);
}
