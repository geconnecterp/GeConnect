...
function accionBotones(btn, tabActiva, dejarCancelar = false) {
    if (btn === AbmAction.ALTA || btn === AbmAction.MODIFICACION || btn === AbmAction.BAJA) {     
        ...
        // El btnDetalle solo se habilita si hay un registro seleccionado
        if (regSelected) {
            $("#btnDetalle").prop("disabled", false);
        } else {
            $("#btnDetalle").prop("disabled", true);
...
function activarControlesSectores(band) {
    if (tipoDeOperacion === "A") {
        $("#Sector_Sec_Id").prop("disabled", !band);
    }
    $("#Sector_Sec_Desc").prop("disabled", !band);
}
...
function btnCancelClick() {
    ...
    // Asegurar que btnFiltro siempre esté habilitado
    $("#btnFiltro").prop("disabled", false);
    ...
    activarControlesSectores(false);
        ...
        PostGen(data, url, function (obj) {
            if (obj.error === true) {
                AbrirMensaje("ATENCIÓN", obj.msg, function () {
                    ...
                    // Añadir estas líneas:
                    $("#btnFiltro").prop("disabled", false);  // Habilitar siempre el btnFiltro

                    // Habilitar el btnDetalle si hay un registro seleccionado
                    if (regSelected && (
                        destinoDeOperacion === AbmObject.CLIENTES ||
                        destinoDeOperacion === AbmObject.PROVEEDORES ||
                        destinoDeOperacion === AbmObject.SECTORES ||
