...
function InicializaPantallaPedido() {
	...
	$("#btnAbmCancelar").on("click", function (e) {
		let msj = "";
		if (modoNuevoPedido) msj = " la carga del pedido";
		if (modoModificacionPedido) msj = " la modificación del pedido";
		if (modoEliminacionPedido) msj = " la baja del pedido";
		AbrirMensaje("ATENCIÓN", `¿Esta seguro que desea cancelar ${msj}?`, function (e) {
			$("#msjModal").modal("hide");
			switch (e) {
				case "SI": //Confirmar 
					cancelarOperacion(e);
...
function agregarProductosAlGrid(productos) {
	...
	// 🔥 actualizar lista global de reemplazables
	actualizarListaReemplazables();

	// 🔥 reconstruir selects existentes
	reconstruirSelectsReemplazo();
...
function crearFilaProductoPedido(producto, esAlternado, pcd_item) {
	...
	return `
        <tr class="${claseAlt}"
			...
			data-permite-decimales="${datosProducto.PermiteDecimales}"
			data-pcd-origen="${datosProducto.pcd_origen}">
...
function actualizarListaReemplazables() {
	const reemplazables = [];

	$('#tbGridPedidoProds tbody tr').each(function () {
		const origen = $(this).data('pcd-origen'); // 'S' o 'N'
		const p_id = $(this).data('p-id');
		const p_desc = $(this).find('.input-p_desc').text().trim();

		if (origen !== 'S') { // NO origen → reemplazable
			reemplazables.push({ p_id, p_desc });
		}
	});

	// eliminar duplicados
	window.productosReemplazables = reemplazables.filter(
		(v, i, a) => a.findIndex(t => t.p_id === v.p_id) === i
	);
}

function reconstruirSelectsReemplazo() {
	$('#tbGridPedidoProds tbody tr').each(function () {
		const $fila = $(this);
		const origen = $fila.data('pcd-origen');
		if (origen !== 'S') return; // solo origen tiene select

		const p_id_actual = $fila.data('p-id');
		const p_id_remplazo = $fila.find('.input-pcd_reemplazo').val() || "";

		const $celda = $fila.find('td').eq(8); // columna Remp
		const nuevoSelect = crearSelectReemplazo(p_id_actual, p_id_remplazo);

		$celda.html(nuevoSelect);
	});
}

function aplicarBloqueoCheckboxOrigen() {
	$('#tbGridPedidoProds tbody tr').each(function () {
		const $fila = $(this);
		const origen = $fila.data('pcd-origen'); // 'S' o 'N'
		const $chk = $fila.find('.input-pcd_origen_bool');
...
function crearSelectReemplazo(p_id_actual, p_id_remplazo) {
	let html = `<select class="form-select form-select-sm input-pcd_reemplazo" style="padding-left: 6px;padding-right: 6px;">
...
function activarTablaProductos() {
	$("#tbGridPedidoProds").removeClass("tabla-desactivada");
	$("#tbGridPedidoProds tbody tr").removeClass("disabled-row");

	// habilitar inputs
	$(".input-pcd_pedida").prop("readonly", false).removeClass("campo-readonly");

	// habilitar selects de reemplazo
	$(".input-pcd_reemplazo").prop("disabled", false);

	// mostrar botones eliminar
	$(".btn-eliminar-producto").show();
}

function desactivarTablaProductos() {
	$("#tbGridPedidoProds").addClass("tabla-desactivada");
	$("#tbGridPedidoProds tbody tr").addClass("disabled-row");

	// bloquear inputs
	$(".input-pcd_pedida").prop("readonly", true).addClass("campo-readonly");

	// bloquear selects
	$(".input-pcd_reemplazo").prop("disabled", true);
...
function cargarProductosPedido(pcCompte, isUpdate = false) {
	...
	PostGenHtml({ pcCompte: pcCompte }, url, function (html) {
		...
		window.productosReemplazables = JSON.parse($("#tbGridPedidoProds").attr("data-reemplazables-json"));
...
function configuracionInputMaskOptimizadaPedido() {
	...
	// Aplicar máscara según PermiteDecimales
	$('#tbGridPedidoProds tbody tr').each(function () {

		const permiteDecimales = $(this).data('permite-decimales');   // true / false
		const $input = $(this).find('.input-pcd_pedida');

		if ($input.length === 0) return;

		if (permiteDecimales === true) {
			// Producto con unidad decimal (up_tipo == "P")
			Inputmask(maskConfig2Decimales).mask($input);
		} else {
			// Producto con unidad entera
			Inputmask({
				alias: "numeric",
				groupSeparator: ",",
				radixPoint: ".",
