...
function inicializaEventosDeVista() {
    $("#chkCondIva").on("click", function () {
        if ($("#chkCondIva").is(":checked")) {
    ...
    $("#CondicionIva").on("change", function () {
        var cond = $("#CondicionIva").val();
        if (cond === 'G') {
            $("#chkAlicIva").prop("disabled", false).prop("checked",false);
        }
        else {
            $("#chkAlicIva").prop("disabled", true);
            $("#AlicuotaIva").val("").prop("disabled", true);
        }

    });

    $("#chkAlicIva").on("click", function () {
        if ($("#chkAlicIva").is(":checked")) {
            $("#AlicuotaIva").prop("disabled", false);
        }
        else {
            $("#AlicuotaIva").prop("disabled", true);
        }
    });
    ...
    $("#btnCancel").on("click", function () {
        window.location.href = homeImpositivo;
    });
...
function buscarDatosImp(btn) {
    ...
    const data = obtenerParamsDatosImpositivos();
...
function obtenerParamsDatosImpositivos() {
   
    // ✅ PASO 1.2: Extraer valores de los selectores
    const conII = $("#chkConII").is(":checked");
    ...
    let data = {
        ...
        Date1: null,
        Date2: null,
        FechaD: null,  // Mantener por compatibilidad
        FechaH: null,  // Mantener por compatibilidad
...
function VerEstadoBtnDetalle() {
    ...
    if ($("#divDetalle").is(":visible") && $("#divDetalle").children().length > 0) {
