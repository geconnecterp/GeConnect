...
// ✅ OPTIMIZADA: Función con parsing seguro de fechas
function obtenerInformacionOfertaDefinida() {
    try {
        // ✅ OBTENER: Valores de los campos del formulario
        var precioOferta = parseFloat($("#txtPrecioOferta").val().replace(/\./g, '').replace(/,/g, '.') || "0");
        var fechaDesde = $("#txtFechaDesde").val();
        var fechaHasta = $("#txtFechaHasta").val();
        var topeVenta = parseInt($("#txtTopeVenta").val() || "0");
        ...
        // ✅ NORMALIZACIÓN: Asegurar que todas las fechas estén normalizadas
        fechaDesdeObj = normalizarFecha(fechaDesdeObj);
        fechaHastaObj = normalizarFecha(fechaHastaObj);
...
function procesarGuardadoTodasLasOfertas(totalProductos, canalesInfo, ofertaInfo) {
    ...
    // ✅ DATOS PARA SERVIDOR: Estructura optimizada para el controlador
    var datosOferta = {
