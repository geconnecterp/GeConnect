...
function inicializaEventosModif() {
    ...
    $("#btnCancel").on("click", function () {
        window.location.href = homeModifPrecioUrl;
    });
...
function buscarProductosPSMP(btn) {
    ...
    if (!usarFechas) {
        AbrirMensaje("Advertencia",
            "Para poder realizar la búsqueda, inicialmente, tiene que activar las fechas del filtro",
    ...
    // Preparar datos que coincidan EXACTAMENTE con TraceReqDto
    // IMPORTANTE: Las propiedades deben tener la primera letra en MAYÚSCULA
    const datos = {
        Desde: date1Value,//fechaDesde.toISOString(),
        Hasta: date2Value,//fechaHasta.toISOString()
    };

    $.ajax({
        url: obtenerProductoTraceUrl,
        type: "POST",
        ...
        error: function (xhr, status, error) {
            ...
            let mensajeError = 'No se pudo obtener la información de productos. Intente nuevamente.';

            // Intentar extraer mensaje de error del servidor
            if (xhr.responseJSON?.mensaje) {
                mensajeError = xhr.responseJSON.mensaje;
            } else if (xhr.status === 400) {
                mensajeError = 'Los datos enviados no son válidos. Verifique las fechas seleccionadas.';
                if (xhr.responseText) {
                    try {
