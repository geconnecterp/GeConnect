...
namespace gc.sitio.Controllers
{
    public class ControladorBase : Controller
    {
        ...
        public List<PresupE> EstadosPresupuesto
        {
            get
            {
                var json = _context.HttpContext?.Session.GetString("EstadosPresupuesto") ?? string.Empty;
                if (string.IsNullOrEmpty(json) || string.IsNullOrWhiteSpace(json))
                {
                    return new List<PresupE>();
                }
                return JsonConvert.DeserializeObject<List<PresupE>>(json) ?? [];
            }
            set
            {
                var json = JsonConvert.SerializeObject(value);
        ...
        protected void ObtenerAdministracionesLista(IAdministracionServicio admServicio, string activa = "%")
        {
        ...
        /// <summary>
        /// Genera una instancia de <see cref="RespuestaGenerica{RespuestaDto}"/> simulando una respuesta genérica para pruebas o mocks.
        /// Permite especificar el tipo de respuesta (error, advertencia, éxito), el mensaje y el código de error.
        /// </summary>
        /// <param name="tipoMock">Tipo de respuesta mock: EsError, EsWarn o EsOk.</param>
        /// <param name="msj">Mensaje descriptivo de la respuesta.</param>
        /// <param name="idError">Código de error asociado a la respuesta.</param>
        /// <returns>Instancia de <see cref="RespuestaGenerica{RespuestaDto}"/> configurada según los parámetros.</returns>
        protected RespuestaGenerica<RespuestaDto> ObtenerRespuestaGenericaMock(TipoDeRespuestaMock tipoMock, string msj = "", short idError = 0)
        {
            var respuesta = new RespuestaGenerica<RespuestaDto>() { EsError = false, EsWarn = false, Ok = false, Mensaje = msj, Entidad = new RespuestaDto() { resultado = idError, resultado_msj = msj, resultado_id = idError.ToString() } };
            switch (tipoMock)
            {
                case TipoDeRespuestaMock.EsError:
                    respuesta.EsError = true;
                    break;
                case TipoDeRespuestaMock.EsWarn:
                    respuesta.EsWarn = true;
                    break;
                case TipoDeRespuestaMock.EsOk:
                    respuesta.Ok = true;
                    break;
                default:
                    break;
