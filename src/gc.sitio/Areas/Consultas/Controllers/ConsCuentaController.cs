...
namespace gc.sitio.Areas.Consultas.Controllers
{
	...
	public class ConsCuentaController : ConsultaControladorBase
	{
		...
		[HttpPost]
		public async Task<IActionResult> ConsultaRecepcionProveedor(string ctaId, DateTime fechaD, DateTime fechaH, string suc)
		{
			...
			try
			{
				...
				if (string.IsNullOrEmpty(suc))
					adm = AdministracionId;
				else
					adm = suc;
				var res = await _consSv.ConsultaRecepcionProveedor(ctaId, fechaD, fechaH, adm, TokenCookie);
				string sort = string.Empty;
				string sortDir = string.Empty;
				int pag = 1;
