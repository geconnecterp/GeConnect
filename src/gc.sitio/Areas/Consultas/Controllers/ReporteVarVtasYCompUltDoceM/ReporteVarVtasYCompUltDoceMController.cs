...
{
	[Area("Consultas")]
	public class ReporteVarVtasYCompUltDoceMController : ReporteVarVtasYCompUltDoceMControladorBase
	{
		...
		private string APP_MODULO = AppModulos.REPORTE_DE_VARIACION_DE_VENTAS_Y_COMPRAS_EN_LOS_ULTIMOS_DOCE_MESES.ToString();
		private readonly IDocManagerServicio _docMSv;

		//************************
		public ReporteVarVtasYCompUltDoceMController(IOptions<AppSettings> options, IHttpContextAccessor contexto, ILogger<ReporteVarVtasYCompUltDoceMController> logger,
		...
		{
			var model = new ReporteVarVtasYCompUltDoceMModel();
			try
			{
				...
				var titulo = "REPORTE DE VARIACIÓN DE VENTAS Y COMPRAS EN LOS ÚLTIMOS DOCE MESES";
		...
		public async Task<IActionResult> RepoVarVtasYCompUltDoceM(ReporteVarVtasYCompUltDoceMRequest request)
		{
			var model = new ListadoVarVtasYCompUltDoceMModel();
			...
			try
			{
				...
				var res = _consultasServicio.RepoVarVtasYCompUltDoceM(request, TokenCookie);
				ListaProductoEvoVtaComp = res;

				model.GrillaProductoEvoVtaComp = ObtenerGridCoreSmart<ReporteVarVtasYCompUltDoceMDto>(res);
				...
				// Leyenda final
				model.Leyenda = $"{model.LeyendaSuc} | {model.LeyendaProv} | {model.LeyendaRub}";
		...
		private void CargarDatosIniciales(ReporteVarVtasYCompUltDoceMModel model)
		{
