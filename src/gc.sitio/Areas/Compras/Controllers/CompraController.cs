	...
	public class CompraController : ControladorBase
	{
		...
		public async Task<IActionResult> VerAut(string rp)
		{
			try
			{
				...
				JsonDeRPVerCompte = ObtenerComprobantesDesdeJson(rp ?? string.Empty);
				...
				{
					throw new NegocioException("No se ha encontrado la cuenta solicitada");
				}
				var detalleVerCompte = await _productoServicio.RPRObtenerItemVerCompte(rp ?? string.Empty, TokenCookie);
				if (detalleVerCompte != null)
				{
					RPRItemVerCompteLista = detalleVerCompte;
				}
				var detalleVerConteos = await _productoServicio.RPRObtenerItemVerConteos(rp ?? string.Empty, TokenCookie);
				...
				model.Leyenda = $"Autorización RP {rp} Cuenta: {cuenta.First().Cta_Denominacion} ({objeto.Cta_id}) Turno: {FormateoDeFecha(objeto.Turno, FechaTipoFormato.PARAUSUARIO)}";
				//Cargar conteos x UL
		...
		public async Task<IActionResult> ObtenerDetalleVerCompte(string rp, string tco_id, string cc_compte)
		{
			...
			try
			{
				...
				{
					detalleVerCompte = await _productoServicio.RPRObtenerItemVerCompte(rp ?? string.Empty, TokenCookie);

					if (detalleVerCompte == null || detalleVerCompte.Count == 0)
					{
						throw new NegocioException("No se ha encontrado detalle del Comprobante.");
					}
		...
		public IActionResult BuscarDetalleVerConteoSeleccionado(string p_id)
		{
			GridCoreSmart<RPRItemVerCompteDto> datosIP = new();
			var lista = new List<RPRItemVerCompteDto>();
			try
		...
		public IActionResult ObtenerDetalleVerConteos()
		{
		...
		public IActionResult ObtenerVerComptesDeRP(string rp)
		{
		...
		private JsonDeRPDto ObtenerComprobantesDesdeJson(string rp)
		{
			...
			try
			{
				...
				var res = _productoServicio.RPObtenerJsonDesdeRP(rp ?? string.Empty, TokenCookie).Result;
				...
				{
					throw new Exception("Hubo algun problema con los comprobantes.");
				}
			}
			catch (Exception ex)
			{
		...
		public async Task<JsonResult> RPREliminarComprobante(string rp)
		{
			...
			try
			{
				respuesta = await _productoServicio.RPREliminarCompte(rp ?? string.Empty, TokenCookie);
				if (respuesta == null)
				{
		...
		public IActionResult NuevaAut(string rp)
		{
			try
			{
				var auth = EstaAutenticado;
				...
				//if (rpSelected == null)
				//{
				//    throw new NegocioException("Hubo un poblema con el RP Seleccionado.");
				//}
				...
				if (!string.IsNullOrWhiteSpace(rp ?? string.Empty))
				{
					RPRAutorizacionPendienteSeleccionadoEnLista = RPRAutorizacionesPendientesEnRP.Where(x => x.Rp == rp).First();
					jsonAux = ObtenerComprobantesDesdeJson(rp ?? string.Empty);
					//RPRComptesDeRPRegs = [];
					//var adms = _tiposComprobantesServicio.BuscarTiposComptesPorCuenta(rpSelected.Cta_id, TokenCookie).GetAwaiter().GetResult();
					if (!string.IsNullOrWhiteSpace(rpSelected?.Cta_id))
					{
						var adms = _tiposComprobantesServicio.BuscarTiposComptesPorCuenta(rpSelected.Cta_id, TokenCookie).GetAwaiter().GetResult();
						TiposComprobantePorCuenta = adms;
					}
					...
					{
						// Manejo del caso en que rpSelected o Cta_id sea nulo o vacío
						throw new ArgumentException("El valor de rpSelected o Cta_id no es válido.");
					}
				...
				else if (RPRAutorizacionSeleccionada != null)
				{
					...
					else
					{
						...
						model.FechaTurno = !string.IsNullOrWhiteSpace(RPRComprobanteDeRPSeleccionado?.FechaTurno) ? Convert.ToDateTime(RPRComprobanteDeRPSeleccionado.FechaTurno).ToString("yyyy-MM-dd") : RPRAutorizacionSeleccionada.Fecha?.ToString("yyyy-MM-dd") ?? string.Empty;
						model.Depo_id = !string.IsNullOrWhiteSpace(RPRComprobanteDeRPSeleccionado?.Depo_id) ? RPRComprobanteDeRPSeleccionado.Depo_id : "0";
						model.CantidadUL = !string.IsNullOrWhiteSpace(RPRComprobanteDeRPSeleccionado?.Ul_cantidad) ? Convert.ToInt32(RPRComprobanteDeRPSeleccionado?.Ul_cantidad) : 0;
						model.Rpe_id = !string.IsNullOrWhiteSpace(RPRAutorizacionSeleccionada?.Rpe_id) ? RPRComprobanteDeRPSeleccionado?.Ul_cantidad ?? string.Empty : "P";
					}
				}
				if (rp == null)
				{
					ViewData["Titulo"] = "NUEVA AUTORIZACIÓN";
		...
		public async Task<IActionResult> CargarDetalleDeProductosEnRP(string oc_compte, string id_prod, string up, string bulto, string unidad, int accion, string tco_id, string cm_compte, string up_id = "", string p_desc = "", string prov_id = "", string id_barrado = "", string listaProd = "")
		{
			...
			else if (!string.IsNullOrWhiteSpace(tco_id) && !string.IsNullOrWhiteSpace(cm_compte))
			{
				...
				if (RPRAutorizacionSeleccionada != null && JsonDeRP != null && JsonDeRP.encabezado.Count <= 0)
				{
					PreCargarJson(RPRAutorizacionSeleccionada.Rp, tco_id, cm_compte);
				}
				if (RPRAutorizacionSeleccionada != null && JsonDeRP != null && JsonDeRP.encabezado.Count > 0 && JsonDeRP.encabezado.First().Rp != RPRAutorizacionSeleccionada.Rp)
				{
					PreCargarJson(RPRAutorizacionSeleccionada.Rp, tco_id, cm_compte);
				}
				if (JsonDeRP != null && JsonDeRP.encabezado != null && JsonDeRP.encabezado.Count > 0)
				{
					...
					if (encTemp != null && encTemp.Comprobantes != null)
					{
						...
						if (!string.IsNullOrWhiteSpace(listaString))
						{
							...
							{
								if (item.Producto == null || item.Producto.P_id == "")
								{
									item.Producto = listaDeProductosAux.Where(x => x.P_id == item.P_id).First();
			...
			try
			{
				if (r.guardado) //Guardo el detalle de productos del compte en la variable de sesion
				{
					...
					if (r.generar)
					{
						...
						else if (JsonDeRP.encabezado.Count == 0)
						{
		...
		public JsonResult ActualizarCuentaComercialSeleccionada(string ctaId, char tipo)
		{
			try
			{
		...
		public ActionResult CargarComprobantesDeRP(string tipo, string tipoDescripcion, string nroComprobante, string fecha, string importe, string rp)
		{
			...
			if (!string.IsNullOrWhiteSpace(rp) && !string.IsNullOrWhiteSpace(tipo) && !string.IsNullOrWhiteSpace(nroComprobante) && (JsonDeRP == null || JsonDeRP.encabezado == null || JsonDeRP.encabezado.Count <= 0))
			{
		...
		private List<RPRComptesDeRPDto> CargarComprobanteRP(List<RPRComptesDeRPDto> listaEnSesion, string tipo, string tipoDescripcion, string nroComprobante, string fecha, string importe, string rp)
		{
			...
			var nuevo = new RPRComptesDeRPDto()
			{
				...
				Importe = importe,
				Importe2 = Convert.ToDecimal(importe.Replace(".", "").Replace(",", ".")),
				Rp = rp
			};
			lista.Add(nuevo);
			return lista;
