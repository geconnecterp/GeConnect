...
namespace gc.sitio.Areas.Mstk.Controllers
{
	...
	public class InventarioReportesController : InventarioReportesControladorBase
	{
		...
		public IActionResult InicializarTabRepoValorDetalle(ReporteInventarioRequest request)
		{
			try
			{
				var auth = EstaAutenticado;
				if (!auth.Item1 || auth.Item2 < DateTime.Now)
					return RedirectToAction("Login", "Token", new { area = "seguridad" });
				request.inv_nro = request.inv_nro == null ? "%" : request.inv_nro;
				var lista = _inventarioServicio.GetReporteValorizadoDetalle(request, TokenCookie);
				var model = new RepoValorDetalleModel
				{
					GrillaRepoValorDetalle = ObtenerGridCoreSmart<InvRepoValorDetalleDto>(lista)
				};
				return PartialView("_gridRepoValorDetalle", model);
			}
			catch (Exception ex)
			{
				RespuestaGenerica<EntidadBase> response = new()
