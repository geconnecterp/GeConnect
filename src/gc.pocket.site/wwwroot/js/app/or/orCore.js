...
function inicializarEventos() {
    ...
    $(OR.dom.tbody).on('click', 'tr[data-or-compte]', function (e) {
        e.preventDefault();
        seleccionarOrden($(this));
    });

    // Evento de botón refrescar (si existe)
    $(OR.dom.btnRefrescar).on('click', function (e) {
        e.preventDefault();
    ...
    $(OR.dom.btnContinuar).on('click', function (e) {
        e.preventDefault();
        continuarConOrden();    
    });
    ...
    let lastTap = 0;
    $(OR.dom.tbody).on('touchend', 'tr[data-or-compte]', function (e) {
...
function cargarOrdenesReparto(forzarRecarga = false) {
    ...
    $.ajax({
        url: OR.endpoints.obtenerOrdenes,
        type: 'POST',
        ...
        beforeSend: function () {
            ...
            $(OR.dom.btnRefrescar).prop('disabled', true);
        },
        ...
        complete: function () {
            ...
            $(OR.dom.btnRefrescar).prop('disabled', false);
...
function seleccionarOrden($row) {
    ...
    $(OR.dom.tbody).find('tr').removeClass('selected');
    ...
    $(OR.dom.btnContinuar).fadeIn(OR.config.animacionDuracion);
...
function validarUsuarioParaOrden(orCompte, usuId) {
    ...
    // Realizar petición AJAX
    $.ajax({
        url: OR.endpoints.validarUsuario,
        type: 'POST',
        data: {
            orCompte: orCompte,
            usuId: usuId
...
function aplicarAnimacionEntrada() {
    const $filas = $(OR.dom.tbody).find('tr[data-or-compte]');
...
Object.assign(window.OR, {
    ...
    limpiarSeleccion: () => {
        ...
        $(OR.dom.btnContinuar).fadeOut(OR.config.animacionDuracion);
    }
});
