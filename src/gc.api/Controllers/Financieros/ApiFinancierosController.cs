...
namespace gc.api.Controllers.Financieros
{
	...
	public class ApiFinancierosController : ControllerBase
	{
		...
		[HttpPost]
		[ProducesResponseType((int)HttpStatusCode.OK, Type = typeof(ApiResponse<LiqDeEmpleadoListaDto>))]
		[ProducesResponseType((int)HttpStatusCode.BadRequest)]
		[Route("[action]")]
		public IActionResult BuscarLiquidacionesDeEmpleados(ConsultaLiqDeEmpleadoRequest request)
		{
			LiqDeEmpleadoListaDto reg = new() { total_paginas = 0, total_registros = 0 };

			_logger.LogInformation($"{GetType().Name} - {MethodBase.GetCurrentMethod()?.Name}");
			var res = _financieroServicio.BuscarLiquidacionesDeEmpleados(request);
			...
			var response = new ApiResponse<IEnumerable<LiqDeEmpleadoListaDto>>(res)
			{
				Meta = metadata
			};
			Response.Headers.Append("X-Pagination", JsonConvert.SerializeObject(metadata));
		...
		[HttpGet]
		[ProducesResponseType((int)HttpStatusCode.OK, Type = typeof(ApiResponse<LiqEmpleadoDetalleParaReporteDto>))]
		[ProducesResponseType((int)HttpStatusCode.BadRequest)]
		[Route("[action]")]
		public IActionResult GetLiqEmpDetalle(string le_compte)
		{
			ApiResponse<List<LiqEmpleadoDetalleParaReporteDto>> response;
			_logger.LogInformation($"{GetType().Name} - {MethodBase.GetCurrentMethod()?.Name}");
			var res = _financieroServicio.GetLiqEmpDetalleParaReporte(le_compte);

			response = new ApiResponse<List<LiqEmpleadoDetalleParaReporteDto>>(res);
