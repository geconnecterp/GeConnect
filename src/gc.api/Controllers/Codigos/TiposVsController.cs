...
namespace gc.api.Controllers.Codigos
{
    ...
    public class TiposVsController : ControllerBase
    {
		...
		[HttpGet]
		[Route("[action]")]
		public IActionResult GetInventarioEstados()
		{
			_logger.LogInformation($"{GetType().Name} - {MethodBase.GetCurrentMethod()?.Name}");
			List<InventarioEstadoDto> condAfip = _inventarioEstadoServicio.GetInventarioEstadoLista();
		...
		[HttpGet]
		[Route("[action]")]
		public IActionResult GetPedidoDeClienteEstados()
		{
			_logger.LogInformation($"{GetType().Name} - {MethodBase.GetCurrentMethod()?.Name}");
			List<PedidoDeClienteEstadoDto> condAfip = _pedidoDeClienteServicio.GetPedidoDeClienteEstados();
