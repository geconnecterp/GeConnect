...
namespace gc.sitio.core.Servicios.Implementacion.ABM
{
    ...
    {
        private const string RUTAAPI = "/api/abmcliente";
        private const string BUSCAR_CLIENTES = "/BuscarClientes";

        private readonly AppSettings _appSettings;
        public ABMClienteServicio(IOptions<AppSettings> options, ILogger<ABMClienteServicio> logger) : base(options, logger, RUTAAPI)
        {
            _appSettings = options.Value;
        }

        public async Task<(List<ABMClienteSearchDto>, MetadataGrid)> BuscarClientes(QueryFilters filters, string token)
        {
            try
            {
                ApiResponse<List<ABMClienteSearchDto>>? apiResponse;
                HelperAPI helper = new();

                HttpClient client = helper.InicializaCliente(filters, token, out StringContent contentData);
                HttpResponseMessage response;

                var link = $"{_appSettings.RutaBase}{RUTAAPI}{BUSCAR_CLIENTES}";
                ...
                if (response.StatusCode == HttpStatusCode.OK)
                {
                    ...
                    apiResponse = JsonConvert.DeserializeObject<ApiResponse<List<ABMClienteSearchDto>>>(stringData);
