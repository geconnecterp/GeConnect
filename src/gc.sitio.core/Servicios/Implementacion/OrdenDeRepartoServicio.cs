...
namespace gc.sitio.core.Servicios.Implementacion
{
	public class OrdenDeRepartoServicio : Servicio<Dto>, IOrdenDeRepartoServicio
	{
		...
		private const string CAMBIA_PRECIOS_EN_ORDEN_DE_REPARTO = "/cambia-precios-or/";
		private const string CAMBIA_ESTADO_ORDEN_DE_REPARTO = "/cambiar-estado-or/";
		...
		public async Task<RespuestaGenerica<RespuestaDto>> APonerEnCursoOrdenDeReparto(APonerEnCursoOrdenDeRepartoRequest req, string token)
		{
			try
			{
				var helper = new HelperAPI();
				var client = helper.InicializaCliente(req, token, out StringContent contentData);
				var link = $"{_appSettings.RutaBase}{RutaAPI}{ANALIZA_PONER_EN_CURSO_ORDEN_DE_REPARTO}";
		...
		public async Task<RespuestaGenerica<RespuestaDto>> CambioDePreciosEnOrdenDeReparto(CambioDePrecioConfirmaRequest req, string token)
		{
			try
			{
				var helper = new HelperAPI();
				var client = helper.InicializaCliente(req, token, out StringContent contentData);
				var link = $"{_appSettings.RutaBase}{RutaAPI}{CAMBIA_PRECIOS_EN_ORDEN_DE_REPARTO}";
			...
			{
				_logger.LogError($"{GetType().Name}-{MethodBase.GetCurrentMethod()?.Name} - {ex}");
				return new() { Ok = false, Mensaje = "Error al CambioDePreciosEnOrdenDeReparto" };
			}
		}

		public async Task<RespuestaGenerica<RespuestaDto>> CambiarEstadoOrdenDeReparto(CambiarEstadoRequest req, string token)
		{
			try
			{
				var helper = new HelperAPI();
				var client = helper.InicializaCliente(req, token, out StringContent contentData);
				var link = $"{_appSettings.RutaBase}{RutaAPI}{CAMBIA_ESTADO_ORDEN_DE_REPARTO}";
			...
			{
				_logger.LogError($"{GetType().Name}-{MethodBase.GetCurrentMethod()?.Name} - {ex}");
				return new() { Ok = false, Mensaje = "Error al CambiarEstadoOrdenDeReparto" };
			}
		}
	}
}
