        ...
        public List<TipoDocumentoDto> GetTipoDocumentoLista(string token)
        {
            try
            {
                ...
                if (response.StatusCode == HttpStatusCode.OK)
                {
                    ...
                    apiResponse = JsonConvert.DeserializeObject<ApiResponse<List<TipoDocumentoDto>>>(stringData) ?? throw new NegocioException("Hubo un problema al deserializar los datos");
