		...
		public List<TipoMonedaDto> ObtenerTipoMoneda(string token)
		{
			try
			{
				...
				if (response.StatusCode == HttpStatusCode.OK)
				{
					...
					apiResponse = JsonConvert.DeserializeObject<ApiResponse<List<TipoMonedaDto>>>(stringData) ?? throw new NegocioException("Hubo un problema al deserializar los datos");
