		...
		public List<TipoCuentaGastoDto> ObtenerTipoCuentaGasto(string token)
		{
			try
			{
				...
				if (response.StatusCode == HttpStatusCode.OK)
				{
					...
					apiResponse = JsonConvert.DeserializeObject<ApiResponse<List<TipoCuentaGastoDto>>>(stringData) ?? throw new NegocioException("Hubo un problema al deserializar los datos");
