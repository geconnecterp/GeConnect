		...
		public List<TipoCuentaBcoDto> GetTipoCuentaBcoLista(string token)
		{
			try
			{
				...
				if (response.StatusCode == HttpStatusCode.OK)
				{
					...
					apiResponse = JsonConvert.DeserializeObject<ApiResponse<List<TipoCuentaBcoDto>>>(stringData) ?? throw new NegocioException("Hubo un problema al deserializar los datos");
