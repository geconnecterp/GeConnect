...
namespace gc.sitio.core.Servicios.Implementacion
{
	public class FinancieroServicio : Servicio<FinancieroDto>, IFinancieroServicio
	{
		...
		private const string ObtenerLiqEmpleadoFileBco = "/FinancieroLiqEmpleadoFileBco";
		private const string GetFinancieroCuentaLista = "/ObtenerFinancieroCuentaLista";
		...
		public List<FinancieroLEProximaDto> GetFinancieroProximaLE(string token)
		{
			try
			{
				ApiResponse<List<FinancieroLEProximaDto>> apiResponse;
				HelperAPI helper = new();
				HttpClient client = helper.InicializaCliente(token);
				HttpResponseMessage response;

				var link = $"{_appSettings.RutaBase}{RutaAPI}{ObtenerFinancieroProximaLE}";
				response = client.GetAsync(link).GetAwaiter().GetResult();
				...
				if (response.StatusCode == HttpStatusCode.OK)
				{
					...
					apiResponse = JsonConvert.DeserializeObject<ApiResponse<List<FinancieroLEProximaDto>>>(stringData) ?? throw new NegocioException("Hubo un problema al deserializar los datos");
					return apiResponse.Data;
				}
				else
				{
		...
		public List<LiqEmpCargaDto> GetLiqEmpCarga(FinancieroLiqEmpCargaRequest request, string token)
		{
			ApiResponse<List<LiqEmpCargaDto>> apiResponse;

			HelperAPI helper = new();
			HttpClient client = helper.InicializaCliente(request, token, out StringContent contentData);
			HttpResponseMessage response;

			var link = $"{_appSettings.RutaBase}{RutaAPI}{SetLiqEmpCarga}";
		...
		public async Task<(List<LiqDeEmpleadoListaDto>, MetadataGrid)> BuscarLiquidacionesDeEmpleados(ConsultaLiqDeEmpleadoRequest filters, string token)
		{
			try
			{
				ApiResponse<List<LiqDeEmpleadoListaDto>>? apiResponse;
				HelperAPI helper = new();

				HttpClient client = helper.InicializaCliente(filters, token, out StringContent contentData);
				HttpResponseMessage response;

				var link = $"{_appSettings.RutaBase}{RutaAPI}{ObtenerLiquidacionesDeEmpleados}";
				...
				if (response.StatusCode == HttpStatusCode.OK)
				{
					...
					apiResponse = JsonConvert.DeserializeObject<ApiResponse<List<LiqDeEmpleadoListaDto>>>(stringData);
		...
		public RespuestaGenerica<RespuestaDto> LiqudacionDeEmpleadoAnular(FinancieroLiqDeEmpleadoAnularRequest request, string token)
		{
			ApiResponse<List<RespuestaDto>> apiResponse;
		...
		public List<LiqEmpleadoFileBcoDto> FinancieroLiqEmpleadoFileBco(FinancieroLiqEmpleadoFileBcoRequest request, string token)
		{
			ApiResponse<List<LiqEmpleadoFileBcoDto>> apiResponse;

			HelperAPI helper = new();
			HttpClient client = helper.InicializaCliente(request, token, out StringContent contentData);
			HttpResponseMessage response;

			var link = $"{_appSettings.RutaBase}{RutaAPI}{ObtenerLiqEmpleadoFileBco}";
			...
			if (response.StatusCode == HttpStatusCode.OK)
			{
				...
				apiResponse = JsonConvert.DeserializeObject<ApiResponse<List<LiqEmpleadoFileBcoDto>>>(stringData) ?? throw new Exception("Error al deserializar la respuesta de la API.");
				return apiResponse.Data;
			}
			else
			{
		...
		public List<FinancieroCuentaListaDto> ObtenerFinancieroCuentaLista(string tcf_id, string token)
		{
			try
			{
				ApiResponse<List<FinancieroCuentaListaDto>> apiResponse;
				HelperAPI helper = new();
				HttpClient client = helper.InicializaCliente(token);
				HttpResponseMessage response;

				var link = $"{_appSettings.RutaBase}{RutaAPI}{GetFinancieroCuentaLista}?tcf_id={tcf_id}";
				response = client.GetAsync(link).GetAwaiter().GetResult();
				...
				if (response.StatusCode == HttpStatusCode.OK)
				{
					...
					apiResponse = JsonConvert.DeserializeObject<ApiResponse<List<FinancieroCuentaListaDto>>>(stringData) ?? throw new NegocioException("Hubo un problema al deserializar los datos");
					return apiResponse.Data;
				}
				else
				{
