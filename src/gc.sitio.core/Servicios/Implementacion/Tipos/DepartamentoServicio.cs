		...
		public List<DepartamentoDto> GetDepartamentoPorProvinciaLista(string prov_id, string token)
		{
			try
			{
				...
				if (response.StatusCode == HttpStatusCode.OK)
				{
					...
					apiResponse = JsonConvert.DeserializeObject<ApiResponse<List<DepartamentoDto>>>(stringData) ?? throw new NegocioException("Hubo un problema al deserializar los datos");
