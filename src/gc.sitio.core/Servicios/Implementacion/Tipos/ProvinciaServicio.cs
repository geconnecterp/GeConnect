		...
		public List<ProvinciaDto> GetProvinciaLista(string token)
		{
			try
			{
				...
				if (response.StatusCode == HttpStatusCode.OK)
				{
					...
					apiResponse = JsonConvert.DeserializeObject<ApiResponse<List<ProvinciaDto>>>(stringData) ?? throw new NegocioException("Hubo un problema al deserializar los datos");
