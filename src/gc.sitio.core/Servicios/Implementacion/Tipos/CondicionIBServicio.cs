		...
		public List<CondicionIBDto> GetCondicionIBLista(string token)
		{
			try
			{
				...
				if (response.StatusCode == HttpStatusCode.OK)
				{
					...
					apiResponse = JsonConvert.DeserializeObject<ApiResponse<List<CondicionIBDto>>>(stringData) ?? throw new NegocioException("Hubo un problema al deserializar los datos");
