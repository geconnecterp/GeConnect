...
$(function () {
    ...
    // A?adir al inicio del documento, después de $(function() {...})
...
function agregarSustitutosAlGrid(sustitutos, productoId) {
    ...
    // A?adir los nuevos sustitutos al mapa, evitando duplicados
...
function cargarProductosCombo(comboId) {
    ...
    $.ajax({
        ...
        success: function (html) {
            ...
            }, 100); // Peque?o retraso para asegurar que el DOM se actualice
        },
        error: function (xhr, status, error) {
            CerrarWaiting();
...
function cargarProductosSustitutos(comboId, productoId) {
    ...
    if (hayEdicionActiva()) {
        console.log("?? Edición activa detectada, no se cargarán sustitutos");
        return false;
    }
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (modoNuevoCombo) {
                ...
                $(".col-sm-6:has(#tbGridSustitutos)").html(response);
...
function guardarRelacionProductoSustitutoEnServidor(productoId) {
    ...
    var sustitutos = productosSustitutosMap[productoId].map(function (sustituto) {
        return {
            ...
            activo: "P"                     // Por defecto pendiente
        };
    });
    //var data = { p_id: productoId, sus: sustitutos };
    //// Enviar al servidor
    //$.ajax({
...
function adaptarGrillaCanales() {
    ...
    // A?adir columna de selección en el encabezado
    $("#tbGridCanales thead tr").prepend("<th class='text-center'>Selección</th>");

    // A?adir checkbox a cada fila
    ...
    // A?adir evento para manejar la selección de canales
